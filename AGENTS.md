...
## Releasing
Every release is a `v*` git tag; GoReleaser builds the linux/amd64 binary and publishes the GitHub Release. The Helm chart's `appVersion` is the default `image.tag`, so it must match the tag — `.github/workflows/release.yml` fails otherwise. Order matters:
